...
- **19.1 Schema migration (`d4e2a7c19f08`)**: added `job_snapshots.tags`, `tagger_version`, `tags_status`, `tags_computed_at`, plus the `job_posting_scores` table (`tenant_id`, `posting_id`, `snapshot_id`, `profile_id`, `profile_version`, `scorer_version`, `agent_version`, `model_id`, `verdict`, `final_score`, `score_breakdown`, `computed_at`) with unique `(tenant_id, snapshot_id, profile_id, profile_version, scorer_version)`. `JobPosting.latest_tags` mirrors the latest snapshot's tags so JobsView avoids the snapshot join. New columns and table carry `tenant_id` from day one.
- **19.2 `src/jobs/tagger.py`**: profile-independent pure-function tagger. `TAGGER_VERSION` constant. Tags: `work_mode`, `level`, `sponsorship_signal`, `intern_eligible`, `posting_age_bucket`, `clearance_required`, `usa_only`. Tests pin the API contract: no profile parameter, no subjective labels (`good_match` / `worth_applying` / `high_priority`).
- **19.3 `posting.tag` + `posting.tag_backfill` Celery tasks**: new tasks routed to the `search` queue. `posting.tag` writes snapshot tags via `src/jobs/tag_service.py`; the production `on_content_changed` listener (`src/jobs/listeners.py`) enqueues a `posting.tag` for every new snapshot. `posting.tag_backfill` pages stale `tagger_version` rows; the operator UI gets a backlog gauge via `GET /api/jobs/tagging/status`.
- **19.3b Saved-search registry fanout**: `search.daily_fanout` now enumerates `config/search_profiles.yaml` and enqueues one `search.refresh` per profile. `search.refresh` accepts either `profile_id` (registry lookup) or `query_id` (legacy single-query) and dispatches `application.jobs.search_jobs(use_job_index=True, force_refresh=True)`. The Phase 18 `not_implemented` stubs are retired.
- **19.4 Score cache write-through**: `src/filter/score_cache.py` exposes `compute_profile_version` (`sha256(canonical_profile)[:12]`), `record_score`, `get_cached_score`. The deterministic scorer in `plan_run._default_score_fn` writes through to `job_posting_scores` after scoring; integrity-error races resolve to the existing row.
- **19.5 `cached_search` refactor**: dropped the TTL short-circuit in `src/jobs/search.py`. `force_refresh` / `freshness_hours` remain on the signature as accepted-but-ignored compatibility knobs. Distributed lock still prevents concurrent same-source scrapes; `search_results` rows still drive "removed since" diffs.
- **19.6 Filter fast-path**: `src/filter/fast_path.py` reads A1 tags + the A2 cache for a snapshot and returns `reject_a1` / `cached` / `miss`. Pending/computing/failed tags always miss (never default-reject). Plan runs now consume A2 cache hits and ready A1 hard-rule rejects before invoking the scorer, then bulk-apply any remaining A1 rejects after scoring for safety. Hard rules derive from `profile.work_authorization.needs_sponsorship` and `profile.clearance`.
- **19.7 Frontend**: `GET /api/jobs/tagging/status` and `POST /api/jobs/postings/{id}/retag` are new endpoints. JobsView shows tag chips (`Remote`, `Senior`, `Intern eligible`, `Sponsorship offered`, `No sponsorship`, `Clearance required`, etc.) per posting, a "Tagging…" badge when the snapshot is still `pending`/`computing`, a `Retag` chip-button to enqueue a one-off refresh, and a backlog banner while the backfill drains. ReviewQueueView surfaces `cached · profile vXYZ · scorer sABC` when a verdict came from the A2 cache.
- **19.8 Docs sweep**: this changelog entry; PROJECT_MANAGEMENT moved to "Phase 19 shipped"; README architecture line updated; D029 already captured the design (2026-05-20) and stands as-is.
- **19.9 Review and deployment hardening**: fixed the wrong profile-id fallback in A1 reject handling; changed A1 reject evaluation to a bulk snapshot-tag lookup; added `FOR UPDATE SKIP LOCKED` to tag backfill selection; isolated per-row review-queue insert failures with nested transactions; removed a dead `usa_only` location branch; bundled the built-in default resume/cover-letter DOCX templates; made default template packages initialize once while respecting later user deletion; made `/api/material-templates` skip invalid packages instead of 500ing; made ProfileView safe when no profile exists; and made scheduled plan runs return `status="no_profile"` before scraping when the scoring profile file is missing.

Verification for the Phase 19 branch after review/deployment hardening: `uv run pytest -q` (1821 passed, 1 skipped), targeted Phase 19/profile/template suites, `npm run build` (existing chunk-size warning), and `git diff --check` all passed.
